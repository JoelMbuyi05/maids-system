...
$stmt = $pdo->prepare("SELECT SUM(price) as total FROM bookings $date_filter");
$stmt->execute($params);
$total_revenue = $stmt->fetch(PDO::FETCH_ASSOC)['total'] ?? 0;

// Calculate total employee payments
$payment_filter = $date_filter ? str_replace('booking_date', 'pay_date', $date_filter) : "";
$stmt = $pdo->prepare("SELECT SUM(amount) as total FROM employee_payments $payment_filter");
$stmt->execute($params);
...
$stmt = $pdo->prepare("SELECT SUM(amount) as total FROM expenses $expense_filter");
$stmt->execute($params);
...
// Fetch employees for payment form
$stmt = $pdo->query("SELECT id, name, emp_number FROM employees ORDER BY name");
$employees = $stmt->fetchAll(PDO::FETCH_ASSOC);

// Monthly revenue data for chart
$stmt = $pdo->query("SELECT DATE_FORMAT(booking_date, '%Y-%m') as month, SUM(price) as total 
                     FROM bookings 
            ...
            <div class="admin-content">
                ...
                <div class="stats-grid">
                    <div class="stat-card revenue">
                        <h3>Total Revenue</h3>
                        <p class="value">R<?= number_format($total_revenue, 2) ?></p>
                    </div>
                    <div class="stat-card expenses">
                        <h3>Employee Payments</h3>
                        <p class="value">R<?= number_format($total_employee_paid, 2) ?></p>
                    </div>
                    <div class="stat-card expenses">
                        <h3>Other Expenses</h3>
                        <p class="value">R<?= number_format($total_expenses, 2) ?></p>
                    </div>
                    <div class="stat-card profit">
                        <h3>Net Profit</h3>
                        <p class="value">R<?= number_format($profit, 2) ?></p>
                    </div>
                </div>
