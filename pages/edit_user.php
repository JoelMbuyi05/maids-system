...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        if (!empty($new_password)) {
            ...
            $stmt = $pdo->prepare("UPDATE users SET name = :name, phone = :phone, address = :address, role = :role, password = :password WHERE id = :id");
            $stmt->execute([
                ':name' => $name,
                ':phone' => $phone,
                ':address' => $address,
            ...
            $stmt = $pdo->prepare("UPDATE users SET name = :name, phone = :phone, address = :address, role = :role WHERE id = :id");
            $stmt->execute([
                ':name' => $name,
                ':phone' => $phone,
                ':address' => $address,
...
<head>
    ...
    <title>Edit User - CleanCare</title>
...
<body>
    ...
    <section class="auth-section-centered" style="padding: 60px 20px;">
        <div class="auth-container-centered" style="max-width: 600px;">
            <h2 style="text-align: center; color: #2c5aa0; margin-bottom: 30px;">✏️ Edit User</h2>
            ...
            <div class="auth-card-compact">
                <form method="POST" class="auth-form-compact">
                    ...
                    <button type="submit" class="btn-submit-compact">Update User</button>
                </form>
